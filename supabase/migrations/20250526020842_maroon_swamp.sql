...
DROP POLICY IF EXISTS "Allow anonymous insert on members" ON members;
DROP POLICY IF EXISTS "Allow authenticated read on members" ON members;
DROP POLICY IF EXISTS "Allow admin all operations on members" ON members;
...
CREATE POLICY "Allow members to read own data"
  ON members
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Allow members to update own data"
  ON members
  FOR UPDATE
  TO authenticated
  USING (id = auth.uid())
