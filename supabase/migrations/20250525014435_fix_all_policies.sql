...
CREATE POLICY "Enable read access for all users on interest_categories"
  ON interest_categories
  FOR SELECT
  TO anon, authenticated
  USING (true);

CREATE POLICY "Enable all access for admins on interest_categories"
  ON interest_categories
  FOR ALL
  TO authenticated
  USING (EXISTS (
...
CREATE POLICY "Enable read access for all users on interests"
  ON interests
  FOR SELECT
  TO anon, authenticated
  USING (true);

CREATE POLICY "Enable all access for admins on interests"
  ON interests
  FOR ALL
  TO authenticated
  USING (EXISTS (
...
CREATE POLICY "Enable all access for admins on members"
  ON members
  FOR ALL
  TO authenticated
  USING (EXISTS (
...
CREATE POLICY "Enable read access for own interests"
  ON member_interests
  FOR SELECT
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM members 
    WHERE members.id = member_interests.member_id 
    AND members.email = auth.jwt() ->> 'email'
  ));

CREATE POLICY "Enable all access for admins on member_interests"
  ON member_interests
  FOR ALL
  TO authenticated
  USING (EXISTS (
